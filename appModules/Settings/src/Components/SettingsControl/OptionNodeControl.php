...
namespace PAF\Modules\Settings\Components\SettingsControl;
...
use PAF\Modules\Settings\InlineOption\OptionAccessor;
...
final class OptionNodeControl extends UI\Control implements OptionAccessor
{
    ...
    private OptionAccessor $optionAccessor;
    private string $optionFqn;
    ...
    public function handleListAvailableValues(): void
    {
        $node = $this->getNode($this->optionFqn);
        if (!$node instanceof IOption) {
    ...
    private function getEditableType($optionType): string
    {
        switch ($optionType) {
    ...
    private function getValueText($value, $type): string
    {
        switch ($type) {
            default:
                return $value;
